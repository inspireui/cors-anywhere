...
function proxyRequest(req, res, proxy) {
  ...
  // Preserve original client IP and entire X-Forwarded-For chain
  var remoteAddress = req.socket.remoteAddress || req.connection.remoteAddress || '1.2.3.4';
  var existingForwardedFor = req.headers['x-forwarded-for'];

  // Build the complete X-Forwarded-For chain
  // If x-forwarded-for already exists, keep it and append the remote address
  // Otherwise, just use the remote address
  var forwardedFor;
  if (existingForwardedFor) {
    // Keep existing chain and append current remote address
    forwardedFor = existingForwardedFor + ', ' + remoteAddress;
  } else {
  ...
  var proxyOptions = {
    ...
    buffer: {
      pipe: function (proxyReq) {
        // Set X-Forwarded-For header on the actual proxy request
        // This ensures it's not overridden by http-proxy's xfwd option
  ...
  tryProxy(req, res, proxy, proxyOptions, 2);
}

function tryProxy(req, res, proxy, proxyOptions, retries = 1) {
  try {
    proxy.web(req, res, proxyOptions)
  } catch (err) {
    if (retries > 0) {
      setTimeout(() => {
        tryProxy(req, res, proxy, proxyOptions, retries - 1);
      }, 100);
    } else {
...
function getHandler(options, proxy) {
  ...
  return function (req, res) {
    ...
    if (x_proxy_domain != null) {
      req.url = '/' + x_proxy_domain;
      delete req.headers['x-proxy-domain']
    }
