...
function parseURL(req_url) {
  ...
  if (!match[1]) {
    ...
    // Default to https when the scheme is omitted, so query-string auth
    // (e.g. WooCommerce consumer_key/secret) works without returning 401.
    req_url = (match[4] === '80' ? 'http:' : 'https:') + req_url;
  }
